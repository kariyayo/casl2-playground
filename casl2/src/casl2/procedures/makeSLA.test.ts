...
describe(`makeSLA`, () => {
  ...
  describe.each([
    {
        tokens: create({ operator: "SLA", operand: "GR1,2" }),
        expected: { wordLength: 2, bytecode: [0x50, 0x10, 2], afterGR1: 0b1000, FR: "000" }
    },
    {
        tokens: create({ operator: "SLA", operand: "GR1,1" }),
        expected: { wordLength: 2, bytecode: [0x50, 0x10, 1], afterGR1: 0b0100, FR: "000" }
    },
    {
        tokens: create({ operator: "SLA", operand: "GR1,1,GR3" }),
        expected: { wordLength: 2, bytecode: [0x50, 0x13, 1], afterGR1: 0b1000, FR: "000" }
    },
    ...
    const res = makeSLA(tokens, flagRegister, grMap)
  ...
  describe.each([
    {
        ...
        tokens: create({ operator: "SLA", operand: "GR2,1" }),
        expected: { bytecode: [0x50, 0x20, 1], afterGR2: 0b0111111111111110, FR: "100" }
    },
    {
        beforeGR2: 0b1111000000000000,
        tokens: create({ operator: "SLA", operand: "GR2,1" }),
        expected: { bytecode: [0x50, 0x20, 1], afterGR2: -8192, FR: "110" }
    },
